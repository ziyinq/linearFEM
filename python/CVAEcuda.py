...
#            torch.nn.init.xavier_uniform(linear.weight)
    ...
    def forward_pass(self):
        ...
        epsilon = Variable(torch.randn(X.shape[0], self.Z_dim)).type(self.dtype)
    ...
    def generate_samples(self, X_star, N_samples):
        ...
        # Reparametrization trick
        epsilon = Variable(torch.randn(N_samples, self.Z_dim)).type(self.dtype)
        ...
        mean_star = mean_star.data.cpu().numpy()
        var_star = var_star.data.cpu().numpy()
        # De-normalize
